...
**Filesystem path:** `/home/tommoseley/dev/TheCombine/`

**Windows access (via WSL):** `\\wsl.localhost\Ubuntu\home\tommoseley\dev\TheCombine`

All relative paths in this document are from this root. When using tools to read files:
- `docs/PROJECT_STATE.md` -> `/home/tommoseley/dev/TheCombine/docs/PROJECT_STATE.md`
- `docs/session_logs/` -> `/home/tommoseley/dev/TheCombine/docs/session_logs/`
- `app/` -> `/home/tommoseley/dev/TheCombine/app/`
...
2. **Use tools to read** `docs/PROJECT_STATE.md` from the filesystem
...
- **Shell Commands**
  - The project runs on WSL/Ubuntu. Bash commands are preferred.
  - PowerShell may be used when accessing from Windows via the WSL network path.
  - When providing commands, prefer bash/Unix syntax.
...
- **HTML Encoding Compliance Check (Mandatory)**
  ...
  - Batch fix script: ops/scripts/fix_html_encoding.ps1 (may need bash equivalent)
...
When working on this project, AI collaborators MUST track technical debt:

1. **When deferring work**: If a task is explicitly deferred to focus on higher priorities, add it to the Technical Debt section in `PROJECT_STATE.md`.

2. **When choosing "good enough"**: If presenting alternatives where one option is quick/pragmatic and another is architecturally correct, and the pragmatic option is chosen, add technical debt to implement the correct approach later.

3. **At session close**: Present the full list of technical debt items added or discovered during the session.

Technical debt entries should include:
- Date identified
- Brief description of the issue
- Preferred/correct approach
...
# From Windows: .\ops\aws\fixip.ps1
