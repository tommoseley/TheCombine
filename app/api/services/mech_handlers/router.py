...
def extract_classification(
    ...
    """Extract classification fields from intake document.

    Pure function — no I/O, no side effects.

    Args:
        intake: Intake document dict
        config: Router config with optional classification_fields

    Returns:
        Classification dict with extracted fields
    """
    classification = {}
...
def score_route(
    ...
    Args:
        route: Route config dict with match conditions
        classification: Extracted classification fields
...
def determine_confidence(
    ...
    """Determine final confidence level for routing decision.

    Pure function — no I/O, no side effects.

    Args:
        winner: Winning candidate dict with score and configured_confidence
        candidates: All candidates sorted by score descending
...
class RouterHandler(MechHandler):
    ...
    async def execute(
        ...
        try:
            ...
            classification = extract_classification(intake, config)
            ...
            for route in routes:
                s = score_route(route, classification)
                candidates.append({
            ...
            winner = candidates[0]
            confidence = determine_confidence(winner, candidates)
