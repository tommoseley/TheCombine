...
@router.get("/executions/{execution_id}/qa-coverage", response_class=HTMLResponse)
async def execution_qa_coverage(
    request: Request,
    execution_id: str,
    db: AsyncSession = Depends(get_db),
):
    """Render QA coverage completeness for a workflow execution.
