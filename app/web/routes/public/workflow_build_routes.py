...
def _parse_pgc_form(form) -> Dict[str, Any]:
    ...
    Delegates parsing to workflow_build_pure.parse_pgc_form_data.
    Keeps logging in the route layer.
    """
    raw_items = list(form.multi_items())
    logger.info(f"PGC form raw items: {raw_items}")
    answers = parse_pgc_form_data(raw_items)
...
def _render_workflow_state(
    ...
    Delegates state classification to workflow_build_pure.classify_workflow_state.
    For HTMX requests, returns just the partial.
    For direct navigation, returns full page with layout.
    """
    config = get_doc_type_display(doc_type_id)
    ...
    # Classify state via pure function
    state_vars = classify_workflow_state(
    ...
    # Map stale_paused to 'failed' for template rendering
    ws = state_vars["workflow_state"]
    if ws == "stale_paused":
        logger.warning(f"Execution {state.execution_id} is paused but has no questions payload - stale state")
        state_vars["workflow_state"] = "failed"

    context.update(state_vars)
