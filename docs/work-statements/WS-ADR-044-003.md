...
1. **Cognitive noise** — User's eye is constantly pulled to artifacts they rarely need to edit
2. **Decision anxiety** — "Do I need to edit this role? That schema?" when the answer is usually no
3. **Wrong mental model** — Suggests the system is assembled bottom-up, when it's governed top-down
4. **Dual editing surfaces** — Gate Internals and Canvas compete for focus, violating the rule that only one locus of complexity may be editable at a time
...
- **ADR-044**: Admin Workbench — defines the three-panel layout
- **ADR-045 §7**: Left rail organized by abstraction level; Building Blocks contain reusable primitives
...
1. WS-ADR-044-001 (Left Rail UX) complete ✓
2. PGC composite gate internals implemented (session 2026-02-06) ✓
3. Schema editor functional ✓

---

## Procedure

### Phase 1: Left Rail — Compositions Only

**Target:** `spa/src/components/admin/DocTypeBrowser.jsx`

**Step 1.1 — Remove Building Blocks section**
...
Keep in left rail:
- **Project Orchestration (POWs)** — grouped by pow_class as currently implemented
- **Document Workflows (DCWs)** — flat alphabetical list with version badges
- **Governance** (Git Status) — unchanged

**Step 1.2 — Update section headers**
...
DOCUMENT WORKFLOWS
  • Concierge Intake       v1.0.0
  • Project Discovery      v1.4.0
...
**Step 2.1 — Create BuildingBlocksTray component**

Create `spa/src/components/admin/BuildingBlocksTray.jsx`:

Props:
- `isOpen`: boolean — tray open state
- `onClose`: function — callback to close tray
- `roles`: array — role fragments
- `templates`: array — templates
- `schemas`: array — schemas
- `fragments`: object — prompt fragments by kind
...
**Step 3.1 — Right panel becomes THE editor**
...
1. Canvas shows selection highlight (glow effect — already implemented)
2. Right panel populates with node properties
3. User edits in right panel only

**Step 3.2 — Remove any inline canvas editing**
...
**Step 4.1 — Single-expansion behavior for PGC internals**
...
**Step 4.2 — Gate-level properties always visible**
...
1. **Compositions are primary** — POWs and DCWs are what users work on
2. **Primitives serve compositions** — Roles, templates, schemas exist to be plugged in, not edited constantly
3. **One focus at a time** — Eliminates cognitive competition between canvas and properties panel
4. **Progressive disclosure** — Complexity hidden until explicitly needed
