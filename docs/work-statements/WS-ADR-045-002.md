...
- [ ] `software_product_development` has `pow_class: reference` and `schema_version: workflow.v2`
- [ ] Backend returns `pow_class`, `derived_from`, `source_version`, `tags` in workflow API responses
- [ ] Backend defaults missing `pow_class` to `reference` for `v1` definitions
- [ ] Left rail groups POWs by class (Reference, Templates)
- [ ] "Create Template from Reference" is the primary creation path
- [ ] "Create Blank" requires an extra step (secondary option)
- [ ] New template POW has `derived_from` pointing to source reference
- [ ] StepWorkflowEditor displays all new metadata fields
- [ ] Tags are editable in the editor
- [ ] All existing navigation and editing still works
- [ ] 1900+ tests still pass
