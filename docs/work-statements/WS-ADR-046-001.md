...
- [x] Migration creates `workflow_instances` and `workflow_instance_history` tables
- [x] Migration rolls back cleanly
- [x] `WorkflowInstance` and `WorkflowInstanceHistory` models load correctly
- [x] Repository CRUD methods work (tier-1 tests)
- [x] Service creates instance with full snapshot from source POW
- [x] Service detects change types on update
- [x] Service computes drift accurately
- [x] API: POST creates instance, returns 201
- [x] API: GET returns instance or 404
- [x] API: PUT updates instance with history
- [x] API: drift endpoint returns accurate comparison
- [x] API: history endpoint returns paginated audit trail
- [x] API: duplicate POST returns 409
- [x] Frontend: workflow picker shows available POWs
- [x] Frontend: instance viewer renders effective workflow
- [x] Frontend: drift indicator works
- [x] Project tree includes workflow status
- [x] FK cascade on project delete removes instance
- [x] All existing tests still pass (1939 passed, 46 skipped)
- [x] Authentication enforced on all new endpoints
