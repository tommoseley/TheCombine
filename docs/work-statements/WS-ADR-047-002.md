...
5. Handle operator submit → workflow resume
...
- [x] Entry type exists in types.yaml with correct schema
- [x] human_input category added to registry
- [x] concierge_entry operation loads via API
- [x] pgc_operator_answers operation loads via API
- [x] EntryComponentRegistry maps op_id to components
- [x] GenericEntryForm renders for unknown operations
- [x] ConciergeEntryForm displays confirmation UI
- [x] PGCAnswerForm displays question/answer UI
- [x] Entry nodes return pending_entry status
- [ ] Frontend loads correct Entry component (deferred - needs workflow executor)
- [ ] Response validation works correctly (deferred - needs workflow executor)
- [x] Node properties panel supports Entry type
