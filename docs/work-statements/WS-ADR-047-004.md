...
- [x] Test coverage for `clarification_merger.py` functions (28 tests)
- [x] Test coverage for `plan_executor.py` transformation methods (17 tests)
...
- [x] Test coverage established for target functions
  - `clarification_merger.py`: 28 tests (existing)
  - `plan_executor.py` transformations: 17 tests (new)
- [x] New handler types registered and working
  - Types added to `types.yaml`: clarification_merger, invariant_pinner, exclusion_filter
  - Handler classes created with `@register_handler` decorator
  - All 6 handlers registered in HANDLER_REGISTRY
- [x] Operation instances created and loading
  - `pgc_clarification_processor`: type=clarification_merger
  - `discovery_invariant_pinner`: type=invariant_pinner
  - `discovery_exclusion_filter`: type=exclusion_filter
- [x] PGC answer processing refactored
  - Two call sites updated to use pgc_clarification_processor operation
  - Fallback to direct function call if operation fails
- [x] Invariant pinning refactored
  - New async wrapper _pin_invariants_via_operation
  - Uses discovery_invariant_pinner operation with fallback
- [x] Exclusion filtering refactored
  - New async wrapper _filter_excluded_via_operation
  - Uses discovery_exclusion_filter operation with fallback
- [x] All existing tests pass (38/38)
- [x] Workflow execution identical before/after (fallback ensures compatibility)
- [x] Dead code marked deprecated
  - `_pin_invariants_to_known_constraints`: deprecated docstring added
  - `_filter_excluded_topics`: deprecated docstring added
- [x] Documentation updated
  - ADR-047 Addendum B: Implementation Report added
  - WS-ADR-047-003 status updated to "Superseded"
