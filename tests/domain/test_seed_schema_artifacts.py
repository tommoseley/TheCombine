...
def test_expected_schema_count():
    ...
    # 4 canonical types (ADR-031) + 4 render model types (ADR-033) + 2 document schemas (ADR-034) + 1 container (ADR-034-EXP) + 2 story schemas (ADR-034-EXP3) + 3 discovery schemas (ADR-034-DISCOVERY) + 1 paragraph + 1 indicator + 1 epic summary + 1 dependencies (ADR-034)
    assert len(INITIAL_SCHEMA_ARTIFACTS) == 20
