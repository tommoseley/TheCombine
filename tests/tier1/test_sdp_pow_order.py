...
Verifies ADR-053 canonical order: Discovery -> IP -> TA -> WP -> WS.

C1: IP precedes TA in graph definition
C2: (removed — IPP/IPF collapsed into single IP by WS-PIPELINE-001)
C3: Regression guard — TA does NOT precede IP
...
class TestADR053CanonicalOrder:
    ...
    def test_ip_precedes_ta(self):
        """C1: implementation_plan precedes technical_architecture."""
        steps = _load_pow_runtime()["steps"]
        ip_idx = _step_index(steps, "implementation_plan")
        ta_idx = _step_index(steps, "technical_architecture")
        assert ip_idx < ta_idx, (
            f"IP (index {ip_idx}) must precede TA (index {ta_idx})"
        )

    def test_ta_does_not_precede_ip(self):
        """C3: Regression guard — TA must NOT precede IP."""
        steps = _load_pow_runtime()["steps"]
        ta_idx = _step_index(steps, "technical_architecture")
        ip_idx = _step_index(steps, "implementation_plan")
        assert ta_idx > ip_idx, (
            f"TA (index {ta_idx}) must NOT precede IP (index {ip_idx})"
        )
